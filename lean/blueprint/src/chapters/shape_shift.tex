...
\begin{remark}[$\tau_\uparrow$ is a valid PBP]
  \label{rem:shapeShiftC_valid}
  The painted diagram $\tau_\uparrow$ produced by any of the four sub-cases
  satisfies all PBP axioms (sym\_P, mono\_P, col\_c\_P, col\_d\_P, row\_s,
  row\_r). This is implicit in the Lean \texttt{shapeShiftC} construction:
  the new paint values are always in the C-type $\mathcal{P}$-allowed set
  $\{\bullet, s, r, c, d\}$; the $r \to c \to d$ layer ordering is preserved
  (case (a.1) moves the $c$ to a new position, with the old $c$ becoming $r$);
  row uniqueness carries over since the C-type $\mathcal{Q}$ only contains
  $\{\bullet, s\}$. Details: [BMSZb] p.~66.
\end{remark}
...
\begin{lemma}[Lemma 10.3 C — PBPSet cardinality]
  ...
  \leanok
  \uses{lem:shapeShiftC_id, rem:shapeShiftC_valid}
