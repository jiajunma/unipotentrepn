...
\begin{definition}[Tail signature $\mathrm{tailSig}(\tau) = (p_{\tau_t}, q_{\tau_t})$]
  ...
  \leanok
  \uses{def:tailContrib, def:tailLen}
...
\begin{lemma}[Tail signature sum identity]
  ...
  \leanok
  \uses{def:tailContrib, def:tailSignature}
...
\begin{lemma}[Prop 11.3 — characterisation of $x_\tau$, $x_1$ via signatures and $\eps$]
  ...
  \leanok
  \uses{def:drc_symbols, def:epsilon_pbp, def:layerOrd, def:sym_allowed, def:tailContrib, def:tailSignature, def:tailSymbol}
...
\begin{lemma}[D-type signature sum]
  ...
  \leanok
  \uses{def:cardCells, def:signature_pbp, def:sym_allowed, lem:fact1_sig_sum}
  For $\tau \in \PBP_D$:
...
\begin{proposition}[Prop 11.4 D — signature decomposition (Eq 11.7)]
  ...
  \leanok
  \uses{def:doubleDescent_D, def:signature_pbp, def:sym_allowed, def:tailSignature}
