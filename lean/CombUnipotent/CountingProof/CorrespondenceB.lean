...
/-- Given σ with Q_bot = r at (hP - 1, 0) (via h_hQσ_eq + h_Qr), shape inclusion,
    and σ.P.shape contains (hP-1, j), then σ.P(hP-1, j) = .c. For j = 0, σ.Q(hP-1, 0) = .r
    is nondot; for j ≥ 1, σ.Q(hP-1, j) = .d by `sigma_Q_eq_d_of_Qbot_r_bal_j_pos`.
    Either way, σ.Q(hP-1, j) ≠ .dot, so σ.P(hP-1, j) ≠ .dot by dot_match, hence = .c by sym_P. -/
private theorem sigma_P_eq_c_of_Qbot_r_bal {μP μQ : YoungDiagram}
    (σ : PBPSet .Bplus μP.shiftLeft μQ.shiftLeft)
    (h_hQσ_eq : μQ.shiftLeft.colLen 0 = μP.colLen 0)
    (h_shape_inc : ∀ j, μP.shiftLeft.colLen j ≤ μQ.shiftLeft.colLen j)
    (h_Qr : σ.val.Q.paint (μQ.shiftLeft.colLen 0 - 1) 0 = .r)
  ...
  have hmemQ : (μP.colLen 0 - 1, j) ∈ σ.val.Q.shape := by
    ...
    rw [σ.prop.2.2, YoungDiagram.mem_iff_lt_colLen]
    exact lt_of_lt_of_le hmemP (h_shape_inc j)
  -- Show σ.Q(hP-1, j) ≠ .dot by case split on j.
  have hQne : σ.val.Q.paint (μP.colLen 0 - 1) j ≠ .dot := by
    by_cases hj : j = 0
    · subst hj
      rw [show σ.val.Q.paint (μP.colLen 0 - 1) 0 = .r from
        (by rw [← h_hQσ_eq]; exact h_Qr)]
      decide
    · have hj_pos : j ≥ 1 := Nat.one_le_iff_ne_zero.mpr hj
      rw [sigma_Q_eq_d_of_Qbot_r_bal_j_pos σ h_hQσ_eq h_Qr j hj_pos hmemQ]
      decide
  -- Then by dot_match (contrapositive), σ.P(hP-1, j) ≠ .dot.
  have hPne : σ.val.P.paint (μP.colLen 0 - 1) j ≠ .dot := by
    intro hPdot
    have := (σ.val.dot_match _ _).mp ⟨hmemP, hPdot⟩
    exact hQne this.2
...
/-! ### Balanced-Qr lift construction

Parallel to `liftPBP_B_bal_Qd` but:
- Takes a `ValidCol0_B_Qr` (so `inr d` has `d.val(0) ∈ {.s}` since DSeq entries are in
  `{.s, .r, .d}` and Qr excludes `.r` / `.d`).
- Replaces `sigma_Q_eq_d_of_Qbot_d_bal` uses with `sigma_Q_eq_d_of_Qbot_r_bal_j_pos`
  (applicable for j ≥ 1).
- For j = 0 at the boundary, uses the fact that `σ.Q(hP-1, 0) = .r` directly.
- For the `inr d` mono_Q col-0 boundary case where d.val(0) could be .r or .d, the
  Qr admissibility constraint forces d.val(0) = .s, making mono_Q trivially satisfied
  (layerOrd .s = 1 ≤ 2 = layerOrd .r = layerOrd σ.Q(hP-1, 0)).
-/

/-- B-type lift construction for balanced case with Q_bot = r.
    Parallel to `liftPBP_B_bal_Qd` but with Qr-specific handling of the
    col-0 boundary row (via ValidCol0_B_Qr admissibility). -/
private noncomputable def liftPBP_B_bal_Qr {μP μQ : YoungDiagram}
    (σ : PBPSet .Bplus (μP.shiftLeft) (μQ.shiftLeft))
    ...
    (h_Qr : σ.val.Q.paint (μQ.shiftLeft.colLen 0 - 1) 0 = .r) :
    PBPSet .Bplus μP μQ := by
  -- Unpack ValidCol0_B_Qr as a ValidCol0_B with extra constraint.
  obtain ⟨v, hv⟩ := v
  -- For inr d in the balanced case (k = μQ.colLen 0 - μP.colLen 0 + 1 ≥ 1),
  -- d.val(0) ≠ .r and ≠ .d; DSeq says d.val(0) ∈ {s,r,d}; so d.val(0) = .s.
  have hk_ge : μQ.colLen 0 - μP.colLen 0 + 1 ≥ 1 := by omega
  -- h_d0_s : for v = inr d, d.val(0) = .s.
  have h_d0_s : ∀ d : DSeq (μQ.colLen 0 - μP.colLen 0 + 1), v = .inr d →
      d.val ⟨0, by omega⟩ = .s := by
    intro d hvd
    rw [hvd] at hv
    simp only at hv
    have ⟨hne_r, hne_d⟩ := hv hk_ge
    rcases d.prop.1 ⟨0, by omega⟩ with h | h | h
    · exact h
    · exact absurd h hne_r
    · exact absurd h hne_d
  have h_Qr' : σ.val.Q.paint (μP.colLen 0 - 1) 0 = .r := by
    rw [← h_hQσ_eq]; exact h_Qr
  have hpoP : ∀ i j, (i, j) ∉ μP → liftPaint_B_P σ.val (μP.colLen 0) v i j = .dot := by
    intro i j hmem; cases j with
  ...
  case mono_P =>
    ...
    | zero =>
      ...
      | succ j₂ =>
        ...
        | inr _ =>
          split_ifs with hc
          · -- i₁ = hP-1, paint = c. Need τ.P(i₂, j₂+1) = σ.P(i₂, j₂) = c.
            -- Derive i₂ = hP-1 from h_weakP, then apply sigma_P_eq_c_of_Qbot_r_bal.
            have h₂mem : i₂ < μP.colLen (j₂ + 1) := YoungDiagram.mem_iff_lt_colLen.mp hmem₂
            have h_wp := h_weakP (j₂ + 1) (by omega)
            have hi₂_lt : i₂ < μP.colLen 0 := lt_of_lt_of_le h₂mem h_wp
            have hi₂_eq : i₂ = μP.colLen 0 - 1 := by omega
            have hmem_σP : (μP.colLen 0 - 1, j₂) ∈ σ.val.P.shape := by
              rw [σ.prop.2.1, YoungDiagram.mem_shiftLeft, ← hi₂_eq]; exact hmem₂
            rw [hi₂_eq, sigma_P_eq_c_of_Qbot_r_bal σ h_hQσ_eq h_shape_inc h_Qr j₂ hmem_σP]
          · simp [DRCSymbol.layerOrd]
    | succ j₁ =>
      cases j₂ with
      | zero => omega
    ...
    | zero =>
      ...
      | succ j₂ =>
        ...
        | inr d =>
          simp only
          by_cases hc : μP.colLen 0 - 1 ≤ i₁ ∧ i₁ < μQ.colLen 0 ∧ μP.colLen 0 > 0
          · -- Balanced case: i₁ could be hP-1. Use h_Qr-based lemmas.
            have h_weak_j : μQ.colLen (j₂+1) ≤ μP.colLen 0 := h_weak (j₂+1) (by omega)
            have hi₂_lt : i₂ < μP.colLen 0 := lt_of_lt_of_le hi₂ h_weak_j
            have hi_eq : i₁ = μP.colLen 0 - 1 := by omega
            have hi₂_eq : i₂ = μP.colLen 0 - 1 := by omega
            have hmemσ : (μP.colLen 0 - 1, j₂) ∈ σ.val.Q.shape := by
              rw [σ.prop.2.2, YoungDiagram.mem_shiftLeft, ← hi₂_eq]; exact hmem₂
            -- Goal: (liftCol0Q_B _ _ (.inr d) i₁).layerOrd ≤ σ.Q(i₂, j₂).layerOrd.
            -- i₁ = hP-1, so d index = 0; v was .inr d so d.val(0) = .s (layerOrd 1).
            rw [dif_pos hc]
            have hi_sub : i₁ - (μP.colLen 0 - 1) = 0 := by omega
            have hd0 := h_d0_s d rfl
            -- Goal: (d.val ⟨i₁ - (μP.colLen 0 - 1), _⟩).layerOrd ≤ σ.Q(i₂, j₂).layerOrd.
            have hdval : d.val ⟨i₁ - (μP.colLen 0 - 1), by omega⟩ = .s := by
              have hfin : (⟨i₁ - (μP.colLen 0 - 1), by omega⟩ : Fin (μQ.colLen 0 - μP.colLen 0 + 1))
                  = ⟨0, hk_ge⟩ :=
                Fin.ext hi_sub
              rw [hfin]; exact hd0
            rw [hdval]
            -- Goal: (.s).layerOrd ≤ σ.Q(i₂, j₂).layerOrd. .s = 1. σ.Q at (hP-1, j₂).
            -- Case on j₂ = 0 or ≥ 1.
            by_cases hj2 : j₂ = 0
            · subst hj2; rw [hi₂_eq]
              rw [show σ.val.Q.paint (μP.colLen 0 - 1) 0 = .r from h_Qr']
              simp [DRCSymbol.layerOrd]
            · have hj2_pos : j₂ ≥ 1 := Nat.one_le_iff_ne_zero.mpr hj2
              rw [hi₂_eq, sigma_Q_eq_d_of_Qbot_r_bal_j_pos σ h_hQσ_eq h_Qr j₂ hj2_pos hmemσ]
              simp [DRCSymbol.layerOrd]
          · rw [dif_neg hc]; simp [DRCSymbol.layerOrd]
    | succ j₁ =>
      cases j₂ with
      ...
      | zero =>
        ...
        | succ j₂ =>
          ...
          have hmemσ : (μP.colLen 0 - 1, j₂) ∈ σ.val.Q.shape := by
            rw [σ.prop.2.2, YoungDiagram.mem_shiftLeft, ← hi_eq]; exact hmemQ
          simp only [liftPaint_B_Q] at h₂
          -- σ.Q(hP-1, j₂) = .s? No: .r (for j₂=0) or .d (for j₂≥1). Either way not .s.
          by_cases hj2 : j₂ = 0
          · subst hj2; rw [hi_eq] at h₂; rw [h_Qr'] at h₂; exact absurd h₂ (by decide)
          · have hj2_pos : j₂ ≥ 1 := Nat.one_le_iff_ne_zero.mpr hj2
            rw [hi_eq, sigma_Q_eq_d_of_Qbot_r_bal_j_pos σ h_hQσ_eq h_Qr j₂ hj2_pos hmemσ] at h₂
            exact absurd h₂ (by decide)
      | succ j₁ =>
        cases j₂ with
        | zero =>
          ...
          simp only [liftPaint_B_Q] at h₁
          by_cases hj1 : j₁ = 0
          · subst hj1; rw [hi_eq] at h₁; rw [h_Qr'] at h₁; exact absurd h₁ (by decide)
          · have hj1_pos : j₁ ≥ 1 := Nat.one_le_iff_ne_zero.mpr hj1
            rw [hi_eq, sigma_Q_eq_d_of_Qbot_r_bal_j_pos σ h_hQσ_eq h_Qr j₁ hj1_pos hmemσ] at h₁
            exact absurd h₁ (by decide)
        | succ j₂ =>
          have := σ.val.row_s i .R .R j₁ j₂
            (show paintBySide σ.val.P σ.val.Q .R i j₁ = .s by simp [paintBySide]; exact h₁)
  ...
  case row_r =>
    ...
    · exact absurd h₁ (hP_no_r i j₁)
    · exact absurd h₂ (hP_no_r i j₂)
    · -- Both τ.Q at col j₁+1 = .r and col j₂+1 = .r. Via σ.row_r get j₁ = j₂.
      -- For j₁=0 or j₂=0 case, derive i = hP-1 and use σ.Q(hP-1, j) structure.
      -- For j₁=succ / j₂=succ (both), use σ.row_r.
      have hQ0_nondot : ∀ x, liftPaint_B_Q σ.val (μP.colLen 0) (μQ.colLen 0) v x 0 ≠ .dot →
          μP.colLen 0 - 1 ≤ x := by
        intro x hne; simp only [liftPaint_B_Q] at hne; unfold liftCol0Q_B at hne
        cases v with
      ...
      | zero =>
        ...
        | succ j₂ =>
          ...
          have hmemσ : (μP.colLen 0 - 1, j₂) ∈ σ.val.Q.shape := by
            rw [σ.prop.2.2, YoungDiagram.mem_shiftLeft, ← hi_eq]; exact hmemQ
          simp only [liftPaint_B_Q] at h₂
          -- h₂: σ.Q(i, j₂) = .r. But σ.Q(hP-1, j₂) = .r only for j₂=0; for j₂≥1, = .d.
          by_cases hj2 : j₂ = 0
          · -- j₂ = 0: σ.Q(hP-1, 0) = .r (consistent with h₂).
            -- h₁: liftPaint_B_Q ... i 0 = .r. We need τ.Q(i, 0) = .r for some valid i.
            -- But for v = .inl d: liftCol0Q_B = d.val(i - hP). d.val ∈ {s,r,d}. = .r possible.
            -- For v = .inr d with Qr: d.val(0) = .s, so liftCol0Q_B(hP-1) = .s ≠ .r.
            -- But i could be > hP-1 and d.val at later index could = .r.
            -- So we need σ.row_r applied between (i, 0) and (i, j₂+1) in τ. But τ has
            -- i = hP-1 forced. d.val(0) for .inr Qr is .s ≠ .r. So liftPaint_B_Q at i=hP-1 ≠ .r.
            subst hj2
            -- i = hP-1 (from hi_eq), so h₁: liftPaint_B_Q at (hP-1, 0) = .r. Contradict.
            rw [hi_eq] at h₁
            simp only [liftPaint_B_Q, liftCol0Q_B] at h₁
            cases v with
            | inl _ => simp at h₁; split_ifs at h₁ with hc; omega
            | inr d =>
              simp at h₁
              split_ifs at h₁ with hc
              · have hd0 := h_d0_s d rfl
                -- h₁ : d.val 0 = .r, hd0 : d.val ⟨0, hk_ge⟩ = .s.
                -- Both refer to index 0 in DSeq. Use Fin.ext to unify.
                have : (0 : Fin (μQ.colLen 0 - μP.colLen 0 + 1)) = ⟨0, hk_ge⟩ := rfl
                rw [this] at h₁; rw [hd0] at h₁; exact absurd h₁ (by decide)
          · have hj2_pos : j₂ ≥ 1 := Nat.one_le_iff_ne_zero.mpr hj2
            rw [hi_eq, sigma_Q_eq_d_of_Qbot_r_bal_j_pos σ h_hQσ_eq h_Qr j₂ hj2_pos hmemσ] at h₂
            exact absurd h₂ (by decide)
      | succ j₁ =>
...
/-- Round-trip: doubleDescent_Bplus_map (liftPBP_B_bal_Qr σ v) = σ.
    Identical to `liftPBP_B_bal_Qd_round_trip` (just passes through σ column data). -/
private theorem liftPBP_B_bal_Qr_round_trip {μP μQ : YoungDiagram}
    (σ : PBPSet .Bplus (μP.shiftLeft) (μQ.shiftLeft))
    (v : ValidCol0_B_Qr (μP.colLen 0) (μQ.colLen 0))
    (hle : μP.colLen 0 ≤ μQ.colLen 0)
    (hP_pos : 0 < μP.colLen 0)
    ...
    (h_Qr : σ.val.Q.paint (μQ.shiftLeft.colLen 0 - 1) 0 = .r) :
    doubleDescent_Bplus_map μP μQ
      (liftPBP_B_bal_Qr σ v hle hP_pos h_weakP h_shape_inc h_hQσ_eq h_weak h_Qr) = σ := by
  set τ := liftPBP_B_bal_Qr σ v hle hP_pos h_weakP h_shape_inc h_hQσ_eq h_weak h_Qr
  apply Subtype.ext
  apply PBP_eq_of_data
  · simp only [doubleDescent_Bplus_map, doubleDescent_B_PBP]; exact σ.prop.1.symm
  · apply PaintedYoungDiagram.ext'
...
/-- liftPBP_B_bal_Qr is injective: different v give different PBPs. -/
private theorem liftPBP_B_bal_Qr_injective {μP μQ : YoungDiagram}
    (σ : PBPSet .Bplus (μP.shiftLeft) (μQ.shiftLeft))
    (hle : μP.colLen 0 ≤ μQ.colLen 0)
    (hP_pos : 0 < μP.colLen 0)
    (h_weakP : ∀ j, j ≥ 1 → μP.colLen j ≤ μP.colLen 0)
    ...
    (h_weak : ∀ j, j ≥ 1 → μQ.colLen j ≤ μP.colLen 0)
    (h_Qr : σ.val.Q.paint (μQ.shiftLeft.colLen 0 - 1) 0 = .r) :
    Function.Injective (fun v : ValidCol0_B_Qr (μP.colLen 0) (μQ.colLen 0) =>
      liftPBP_B_bal_Qr σ v hle hP_pos h_weakP h_shape_inc h_hQσ_eq h_weak h_Qr) := by
  intro v₁ v₂ h
  apply Subtype.ext
  have hval : (liftPBP_B_bal_Qr σ v₁ hle hP_pos h_weakP h_shape_inc h_hQσ_eq h_weak h_Qr).val =
              (liftPBP_B_bal_Qr σ v₂ hle hP_pos h_weakP h_shape_inc h_hQσ_eq h_weak h_Qr).val :=
    congrArg Subtype.val h
  have hPeq : ∀ i, liftPaint_B_P σ.val (μP.colLen 0) v₁.val i 0 =
                    liftPaint_B_P σ.val (μP.colLen 0) v₂.val i 0 := by
    intro i
    exact congr_fun (congr_fun (congrArg (fun d => d.P.paint) hval) i) 0
  have hQeq : ∀ i, liftPaint_B_Q σ.val (μP.colLen 0) (μQ.colLen 0) v₁.val i 0 =
                    liftPaint_B_Q σ.val (μP.colLen 0) (μQ.colLen 0) v₂.val i 0 := by
    intro i
  ...
  obtain ⟨v₁, hv₁⟩ := v₁
  obtain ⟨v₂, hv₂⟩ := v₂
  -- Goal reduces (via Subtype.ext) to v₁ = v₂.
  match v₁, v₂ with
  | .inl d₁, .inl d₂ =>
    show Sum.inl d₁ = Sum.inl d₂
    congr 1; apply Subtype.ext; funext ⟨k, hk⟩
    have hq := hQeq (μP.colLen 0 + k)
    have hlt : μP.colLen 0 + k < μQ.colLen 0 := by omega
    simp only [dif_pos (show μP.colLen 0 ≤ μP.colLen 0 + k ∧ μP.colLen 0 + k < μQ.colLen 0
...
private theorem validCol0_B_Qr_le_fiber {r₁ r₂ : ℕ} {rest : DualPart} {μP μQ : YoungDiagram}
    ...
    Fintype.card (doubleDescent_Bplus_fiber σ) := by
  -- Extract shape relations (mirror fiber_card_B_bal_Qd pattern).
  have hP0 : μP.colLen 0 = r₂ / 2 :=
    colLen_0_eq_of_colLens_cons (by rw [hP, dpartColLensP_B_cons₂])
  have hQ0 : μQ.colLen 0 = r₁ / 2 :=
    colLen_0_eq_of_colLens_cons (by rw [hQ, dpartColLensQ_B_cons₂])
  ...
  -- Apply injection via liftPBP_B_bal_Qr.
  apply Fintype.card_le_of_injective
    (fun v => ⟨liftPBP_B_bal_Qr σ v hle hP_pos h_weakP h_shape_inc h_hQσ_eq h_weak h_Qr,
              liftPBP_B_bal_Qr_round_trip σ v hle hP_pos h_weakP h_shape_inc h_hQσ_eq h_weak h_Qr⟩)
  intro v₁ v₂ h
  exact liftPBP_B_bal_Qr_injective σ hle hP_pos h_weakP h_shape_inc h_hQσ_eq h_weak h_Qr
    (congrArg (fun x => x.val) h)
...
/-! ### Sub-fiber card lemmas (topSym.lo ≤ 1) on ValidCol0_B_Qr / ValidCol0_B_Qlow

Mirror of `validCol0_B_card_top_lo_le_one` for the restricted subtypes used in the
balanced Qr and Qlow cases. -/

/-- |ValidCol0_B_Qr with topSym.layerOrd ≤ 1| = 2.
    The Qr predicate excludes only two elements (both `inr d` with d.val(0) ∈ {r, d}):
    `DSeq_allR` (last = r, layerOrd 2) and `DSeq_Rd` (last = d, layerOrd 4). Neither
    has topSym.layerOrd ≤ 1, so the restriction does not shrink the lo≤1 set.
    The result equals `validCol0_B_card_top_lo_le_one = 2`. -/
private theorem validCol0_B_Qr_card_top_lo_le_one (hP hQ : ℕ) (hle : hP ≤ hQ)
    (k : ℕ) (hk : k = hQ - hP + 1) (hk_pos : k ≥ 1) :
    Fintype.card {v : ValidCol0_B_Qr hP hQ // (topSym_B hP hQ v.val).layerOrd ≤ 1} = 2 := by
  have hequiv : {v : ValidCol0_B_Qr hP hQ // (topSym_B hP hQ v.val).layerOrd ≤ 1} ≃
      {v : ValidCol0_B hP hQ // (topSym_B hP hQ v).layerOrd ≤ 1} := by
    refine {
      toFun := fun ⟨⟨v, _⟩, hlo⟩ => ⟨v, hlo⟩
      invFun := fun ⟨v, hlo⟩ => ⟨⟨v, ?_⟩, hlo⟩
      left_inv := ?_
      right_inv := ?_
    }
    · cases v with
      ...
      | inr d =>
        intro _
        have htop : d.val ⟨hQ - hP, by omega⟩ = .s := by
          have : (topSym_B hP hQ (Sum.inr d)).layerOrd ≤ 1 := hlo
          simp only [topSym_B] at this
          rcases d.prop.1 ⟨hQ - hP, by omega⟩ with h | h | h
          · exact h
          · rw [h, DRCSymbol.layerOrd] at this; omega
          · rw [h, DRCSymbol.layerOrd] at this; omega
        have hmono := d.prop.2.1 ⟨0, by omega⟩ ⟨hQ - hP, by omega⟩ (by simp)
        rw [htop] at hmono
        rcases d.prop.1 ⟨0, by omega⟩ with h | h | h
        · exact ⟨by rw [h]; decide, by rw [h]; decide⟩
        · rw [h, DRCSymbol.layerOrd, DRCSymbol.layerOrd] at hmono; omega
        · rw [h, DRCSymbol.layerOrd, DRCSymbol.layerOrd] at hmono; omega
    · rintro ⟨⟨v, _⟩, _⟩; rfl
    · rintro ⟨v, _⟩; rfl
  rw [Fintype.card_congr hequiv]
  exact validCol0_B_card_top_lo_le_one hP hQ hle k hk hk_pos

/-- |ValidCol0_B_Qlow with topSym.layerOrd ≤ 1| = 1.
    For k = 1: topSym = .dot always, count = |DSeq(0)| = 1.
    For k ≥ 2: last = .s forces DSeq_const_s, count = 1. -/
private theorem validCol0_B_Qlow_card_top_lo_le_one (hP hQ : ℕ) (hle : hP ≤ hQ)
    (k : ℕ) (hk : k = hQ - hP + 1) (hk_pos : k ≥ 1) :
    Fintype.card {v : ValidCol0_B_Qlow hP hQ // (topSym_B hP hQ v.val).layerOrd ≤ 1} = 1 := by
  have hequiv : {v : ValidCol0_B_Qlow hP hQ // (topSym_B hP hQ v.val).layerOrd ≤ 1} ≃
      {d : DSeq (hQ - hP) // (topSym_B hP hQ (Sum.inl d)).layerOrd ≤ 1} := by
    refine {
      toFun := fun ⟨⟨v, hv⟩, hlo⟩ => ?_
      invFun := fun ⟨d, hd⟩ => ⟨⟨Sum.inl d, trivial⟩, hd⟩
      left_inv := ?_
      right_inv := ?_
    }
  ...
  by_cases hK : hQ - hP ≥ 1
  · have hsub : Fintype.card {d : DSeq (hQ - hP) //
        (topSym_B hP hQ (Sum.inl d)).layerOrd ≤ 1} =
        Fintype.card {d : DSeq (hQ - hP) // d.val ⟨hQ - hP - 1, by omega⟩ = .s} := by
      apply Fintype.card_congr
      refine {
        toFun := fun ⟨d, hd⟩ => ⟨d, ?_⟩
        invFun := fun ⟨d, hs⟩ => ⟨d, ?_⟩
      ...
      · simp only [topSym_B] at hd
        rw [dif_pos hK] at hd
        rcases d.prop.1 ⟨hQ - hP - 1, by omega⟩ with h | h | h
        · exact h
        · rw [h, DRCSymbol.layerOrd] at hd; omega
        · rw [h, DRCSymbol.layerOrd] at hd; omega
      · show (topSym_B hP hQ (Sum.inl d)).layerOrd ≤ 1
        simp only [topSym_B]; rw [dif_pos hK, hs, DRCSymbol.layerOrd]
      · rintro ⟨d, _⟩; rfl
      · rintro ⟨d, _⟩; rfl
    rw [hsub, card_DSeq_last_s hK]
  · have h_all : ∀ d : DSeq (hQ - hP), (topSym_B hP hQ (Sum.inl d)).layerOrd ≤ 1 := by
      intro d
      simp only [topSym_B]; rw [dif_neg hK, DRCSymbol.layerOrd]; omega
    rw [show Fintype.card {d : DSeq (hQ - hP) //
        (topSym_B hP hQ (Sum.inl d)).layerOrd ≤ 1} = Fintype.card (DSeq (hQ - hP)) from by
      apply Fintype.card_congr
      exact {
        toFun := fun ⟨d, _⟩ => d
        ...
        right_inv := fun _ => rfl }]
    rw [DSeq_card]
    have : hQ - hP = 0 := by omega
    omega

/-! ### Refined fiber counts: sub-fiber with new-level `Q_bot.layerOrd ≤ 1`

Mirror of DD-direction refined fiber count infra (for topSym = .d) for the
SS-direction (topSym.layerOrd ≤ 1). Routes through the respective
`validCol0_B_*_card_top_lo_le_one` lemmas. -/

/-- **Refined fiber count (Q_bot = d, topSym.lo ≤ 1)**: For σ with Q_bot = d, the
    sub-subtype of fibers with `(new_Q_bot).layerOrd ≤ 1` has size 2. -/
private theorem fiber_alpha_topSym_lo_le_one_count_bal_Qd {r₁ r₂ : ℕ} {rest : DualPart}
    {μP μQ : YoungDiagram}
    (hP : μP.colLens = dpartColLensP_B (r₁ :: r₂ :: rest))
    (hQ : μQ.colLens = dpartColLensQ_B (r₁ :: r₂ :: rest))
    (hsort : (r₁ :: r₂ :: rest).SortedGE)
    ...
    (h_Qd : σ.val.Q.paint (μQ.shiftLeft.colLen 0 - 1) 0 = .d) :
    Fintype.card {τ : doubleDescent_Bplus_fiber σ //
      (τ.val.val.Q.paint (μQ.colLen 0 - 1) 0).layerOrd ≤ 1} = 2 := by
  have hP0 : μP.colLen 0 = r₂ / 2 :=
    colLen_0_eq_of_colLens_cons (by rw [hP, dpartColLensP_B_cons₂])
  have hQ0 : μQ.colLen 0 = r₁ / 2 :=
    colLen_0_eq_of_colLens_cons (by rw [hQ, dpartColLensQ_B_cons₂])
  ...
  rw [show (2 : ℕ) =
        Fintype.card {v : ValidCol0_B (μP.colLen 0) (μQ.colLen 0) //
          (topSym_B (μP.colLen 0) (μQ.colLen 0) v).layerOrd ≤ 1} from by
    rw [validCol0_B_card_top_lo_le_one _ _ hle _ hk_eq hk_pos]]
  apply le_antisymm
  · apply Fintype.card_le_of_injective
      (fun (x : {τ : doubleDescent_Bplus_fiber σ //
...
/-- **Refined fiber count (Q_bot = r, topSym.lo ≤ 1)**: For σ with Q_bot = r, the
    sub-subtype of fibers with `(new_Q_bot).layerOrd ≤ 1` has size 2. -/
private theorem fiber_alpha_topSym_lo_le_one_count_bal_Qr {r₁ r₂ : ℕ} {rest : DualPart}
    {μP μQ : YoungDiagram}
    (hP : μP.colLens = dpartColLensP_B (r₁ :: r₂ :: rest))
    (hQ : μQ.colLens = dpartColLensQ_B (r₁ :: r₂ :: rest))
    (hsort : (r₁ :: r₂ :: rest).SortedGE)
  ...
  rw [show (2 : ℕ) =
        Fintype.card {v : ValidCol0_B_Qr (μP.colLen 0) (μQ.colLen 0) //
          (topSym_B (μP.colLen 0) (μQ.colLen 0) v.val).layerOrd ≤ 1} from by
    rw [validCol0_B_Qr_card_top_lo_le_one _ _ hle _ hk_eq hk_pos]]
  apply le_antisymm
  · apply Fintype.card_le_of_injective
      (fun (x : {τ : doubleDescent_Bplus_fiber σ //
          (τ.val.val.Q.paint (μQ.colLen 0 - 1) 0).layerOrd ≤ 1}) =>
...
/-- **α-class DD count**: combined B⁺ ∪ B⁻ PBPs with Q column 0 bottom = d
    ...
    Previously had a scoped sorry for the balanced case. Now closed by delegating
    to `card_B_combined` which proves Total + A1 + A3 jointly via structural
    induction. The `h_total` parameter is kept for backward compatibility with
    existing callers but is now unused (Total is derived internally). -/
private theorem card_B_DD_alpha_eq_countB_dd (dp : DualPart)
    ...
    (Finset.univ.filter fun σ : PBPSet .Bminus μP μQ =>
      σ.val.Q.paint (μQ.colLen 0 - 1) 0 = .d).card =
      (countPBP_B dp).1 :=
  ((card_B_combined dp hP hQ hsort heven hpos).2 hQ_pos).1
...
/-- **α-class SS count**: B⁻ PBPs with Q column 0 bottom ∈ {•, s}
    ...
    Previously had a scoped sorry for the inductive case. Now closed by delegating
    to `card_B_combined`. -/
theorem card_B_SS_alpha_eq_countB_ss (dp : DualPart)
    ...
    (Finset.univ.filter fun σ : PBPSet .Bminus μP μQ =>
      (σ.val.Q.paint (μQ.colLen 0 - 1) 0).layerOrd ≤ 1).card =
      (countPBP_B dp).2.2 :=
  ((card_B_combined dp hP hQ hsort heven hpos).2 hQ_pos).2
