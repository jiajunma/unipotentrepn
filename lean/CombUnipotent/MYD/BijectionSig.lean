...
/-- **Φ_D_sig is injective** (paper Prop 11.15 D-type).

    Requires non-empty shape — for empty (μP, μQ), the map sends both
    ε values of the unique empty τ to the same empty MYD_sig. -/
