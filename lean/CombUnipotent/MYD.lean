...
/-- **Lemma 11.5 D-type via Prop 11.4 shape identity**:

    Replaces h_pt / h_qt with the Prop 11.4 signature decomposition
      p = c_2(O) + p_prev + p_t  (outer B/D at τ, via Prop 11.4)
    combined with the **shape-level identity**
      2 · n_inner = c_2(O) + p_prev + q_prev  (from BV dual + single descent cell count)

    The shape identity reflects that n_inner (the inner C/M signature) equals
    |τ'| = |τ| - c_1(O), and combining with |τ| expressions from PBP.signature
    gives the desired arithmetic relation. -/
theorem ILS.lemma_11_5_D_from_prop_11_4 (E : ILS) (c_2 n_inner n_prev p q p_t q_t : ℤ)
    (p_prev q_prev : ℤ)
    (h_sign : ILS.sign E = (p_prev, q_prev))
    (h_fc : ILS.firstColSign E = (p_prev - n_prev, q_prev - n_prev))
    (h_n₀ : n_inner - p_prev - q_prev + n_prev ≥ 0)
    -- Prop 11.4 signature decomposition (for outer B/D τ, primitive case)
    (prop_11_4_p : p - p_t = c_2 + p_prev)
    (prop_11_4_q : q - q_t = c_2 + q_prev)
    -- Shape identity: 2·n_inner = c_2 + p_prev + q_prev (from BV dual + cell count)
    (shape_id : 2 * n_inner = c_2 + p_prev + q_prev)
    (γ₁ : ℤ) :
    let n₀ := n_inner - (ILS.sign E).1 - (ILS.sign E).2 + n_prev
    let inner := ILS.charTwistCM (ILS.augment (n₀, n₀) E) γ₁
    p - (ILS.sign inner).1 - (ILS.firstColSign inner).2 = p_t ∧
    q - (ILS.sign inner).2 - (ILS.firstColSign inner).1 = q_t := by
  have h_pt : p - 2 * n_inner + (ILS.sign E).2 = p_t := by
    rw [h_sign]; simp; omega
  have h_qt : q - 2 * n_inner + (ILS.sign E).1 = q_t := by
    rw [h_sign]; simp; omega
  have h_fc' : ILS.firstColSign E = ((ILS.sign E).1 - n_prev, (ILS.sign E).2 - n_prev) := by
    rw [h_sign]; exact h_fc
/-! ### Lemma 11.6 PBP-level first entry formula

Combining the firstColSign invariant with `ILS.thetaLift_{CD,MB}_first_entry`
and the B/D post-twist `ILS.twistBD_first_entry`, every r in the outer AC.step
output has a specific first entry.

When the source `L_{τ'}` has uniform sign `(n_inner, n_inner)` (C/M-type) and
firstColSign `(n_inner - n_prev, n_inner - n_prev)` (the invariant), we get:
  `r.head? = some (p - 2n_inner + n_prev, (-1)^{ε_τ} · (q - 2n_inner + n_prev))`

Via the shape-level identities `h_pt, h_qt`, this further simplifies to
`(p_t, (-1)^{ε_τ} · q_t)` = (paper-level tail signature). -/

/-- After the B/D post-twist ⊗(0, ε_τ), first entry `(a, b)` becomes `(a, (-1)^{ε_τ} · b)`. -/
theorem ACResult.postTwist_BD_first_entry (ac : ACResult) (ε_τ : Fin 2) (a b : ℤ)
  ...
  · rename_i hε
    simp only [ACResult.twistBD, List.mem_map] at hr
    obtain ⟨⟨c, ils⟩, hmem, rfl⟩ := hr
    have := h_first ⟨c, ils⟩ hmem
    simp only at *
    rw [ILS.twistBD_first_entry, this]
    simp [hε]
  · rename_i hε
    simp [hε]
    exact h_first r hr

/-- **Lemma 11.6 (D-type, unconditional at AC level):**
    The outer AC.step (γ = D) produces `L_τ` whose every r has first entry
    `(p - 2n_inner + n_prev, (-1)^{ε_τ} · (q - 2n_inner + n_prev))`. -/
theorem AC.lemma_11_6_D_unconditional (source : ACResult) (n_inner n_prev p q : ℤ)
    (ε_τ ε_wp : Fin 2)
    (h_sign : ∀ r ∈ source, ILS.sign r.2 = (n_inner, n_inner))
    (h_fc : ∀ r ∈ source, ILS.firstColSign r.2 = (n_inner - n_prev, n_inner - n_prev))
    (h_std : ∀ r ∈ source,
      p - (ILS.sign r.2).1 - (ILS.firstColSign r.2).2 ≥ 0 ∧
      q - (ILS.sign r.2).2 - (ILS.firstColSign r.2).1 ≥ 0) :
    ∀ r ∈ AC.step source .D p q ε_τ ε_wp,
      r.2.head? = some (p - 2 * n_inner + n_prev,
                         if ε_τ = 1 then -(q - 2 * n_inner + n_prev)
                         else q - 2 * n_inner + n_prev) := by
  intro r hr
  simp only [AC.step,
    show ¬(RootType.D = RootType.C ∨ RootType.D = RootType.M) from by decide,
    ite_false,
    show (RootType.D = RootType.Bplus ∨ RootType.D = RootType.Bminus ∨ RootType.D = RootType.D)
      from Or.inr (Or.inr rfl), true_and] at hr
  have h_lift_first : ∀ r ∈ source.thetaLift RootType.D p q,
      r.2.head? = some (p - 2 * n_inner + n_prev, q - 2 * n_inner + n_prev) := by
    intro r' hr'
    simp only [ACResult.thetaLift, List.mem_flatMap, List.mem_map] at hr'
    obtain ⟨⟨c, ils⟩, hmem, lifted, hlift, rfl⟩ := hr'
    simp only
    simp only [ILS.thetaLift] at hlift
    have h := ILS.thetaLift_CD_first_entry ils p q (h_std ⟨c, ils⟩ hmem) lifted hlift
    rw [h, h_sign ⟨c, ils⟩ hmem, h_fc ⟨c, ils⟩ hmem]
    simp; omega
  exact ACResult.postTwist_BD_first_entry _ ε_τ _ _ h_lift_first r hr

/-- **Lemma 11.6 (B⁺ variant):** Same formula for γ = B⁺ (via M→B lift). -/
theorem AC.lemma_11_6_Bplus_unconditional (source : ACResult) (n_inner n_prev p q : ℤ)
    (ε_τ ε_wp : Fin 2)
    (h_sign : ∀ r ∈ source, ILS.sign r.2 = (n_inner, n_inner))
    (h_fc : ∀ r ∈ source, ILS.firstColSign r.2 = (n_inner - n_prev, n_inner - n_prev))
    (h_std : ∀ r ∈ source,
      p - (ILS.sign r.2).1 - (ILS.firstColSign r.2).2 ≥ 0 ∧
      q - (ILS.sign r.2).2 - (ILS.firstColSign r.2).1 ≥ 0) :
    ∀ r ∈ AC.step source .Bplus p q ε_τ ε_wp,
      r.2.head? = some (p - 2 * n_inner + n_prev,
                         if ε_τ = 1 then -(q - 2 * n_inner + n_prev)
                         else q - 2 * n_inner + n_prev) := by
  intro r hr
  simp only [AC.step,
    show ¬(RootType.Bplus = RootType.C ∨ RootType.Bplus = RootType.M) from by decide,
    ite_false,
    show (RootType.Bplus = RootType.Bplus ∨ RootType.Bplus = RootType.Bminus ∨
          RootType.Bplus = RootType.D) from Or.inl rfl, true_and] at hr
  have h_lift_first : ∀ r ∈ source.thetaLift RootType.Bplus p q,
      r.2.head? = some (p - 2 * n_inner + n_prev, q - 2 * n_inner + n_prev) := by
    intro r' hr'
    simp only [ACResult.thetaLift, List.mem_flatMap, List.mem_map] at hr'
    obtain ⟨⟨c, ils⟩, hmem, lifted, hlift, rfl⟩ := hr'
    simp only
    simp only [ILS.thetaLift] at hlift
    have h := ILS.thetaLift_MB_first_entry ils p q (h_std ⟨c, ils⟩ hmem) lifted hlift
    rw [h, h_sign ⟨c, ils⟩ hmem, h_fc ⟨c, ils⟩ hmem]
    simp; omega
  exact ACResult.postTwist_BD_first_entry _ ε_τ _ _ h_lift_first r hr

/-- **Lemma 11.6 (B⁻ variant):** Same formula for γ = B⁻. -/
theorem AC.lemma_11_6_Bminus_unconditional (source : ACResult) (n_inner n_prev p q : ℤ)
    (ε_τ ε_wp : Fin 2)
    (h_sign : ∀ r ∈ source, ILS.sign r.2 = (n_inner, n_inner))
    (h_fc : ∀ r ∈ source, ILS.firstColSign r.2 = (n_inner - n_prev, n_inner - n_prev))
    (h_std : ∀ r ∈ source,
      p - (ILS.sign r.2).1 - (ILS.firstColSign r.2).2 ≥ 0 ∧
      q - (ILS.sign r.2).2 - (ILS.firstColSign r.2).1 ≥ 0) :
    ∀ r ∈ AC.step source .Bminus p q ε_τ ε_wp,
      r.2.head? = some (p - 2 * n_inner + n_prev,
                         if ε_τ = 1 then -(q - 2 * n_inner + n_prev)
                         else q - 2 * n_inner + n_prev) := by
  intro r hr
  simp only [AC.step,
    show ¬(RootType.Bminus = RootType.C ∨ RootType.Bminus = RootType.M) from by decide,
    ite_false,
    show (RootType.Bminus = RootType.Bplus ∨ RootType.Bminus = RootType.Bminus ∨
          RootType.Bminus = RootType.D) from Or.inr (Or.inl rfl), true_and] at hr
  have h_lift_first : ∀ r ∈ source.thetaLift RootType.Bminus p q,
      r.2.head? = some (p - 2 * n_inner + n_prev, q - 2 * n_inner + n_prev) := by
    intro r' hr'
    simp only [ACResult.thetaLift, List.mem_flatMap, List.mem_map] at hr'
    obtain ⟨⟨c, ils⟩, hmem, lifted, hlift, rfl⟩ := hr'
...
/-- **Lemma 11.6 (unified B/D):** For γ ∈ {D, B⁺, B⁻}, every r ∈ AC.step source γ p q ε_τ ε_wp
    has first entry `(p - 2n_inner + n_prev, (-1)^{ε_τ} · (q - 2n_inner + n_prev))`. -/
theorem AC.lemma_11_6_BD_unconditional (source : ACResult) (n_inner n_prev p q : ℤ)
    (γ : RootType) (hγ : γ = .D ∨ γ = .Bplus ∨ γ = .Bminus)
    (ε_τ ε_wp : Fin 2)
    (h_sign : ∀ r ∈ source, ILS.sign r.2 = (n_inner, n_inner))
    (h_fc : ∀ r ∈ source, ILS.firstColSign r.2 = (n_inner - n_prev, n_inner - n_prev))
  ...
  rcases hγ with rfl | rfl | rfl
  · exact AC.lemma_11_6_D_unconditional source n_inner n_prev p q ε_τ ε_wp h_sign h_fc h_std
  · exact AC.lemma_11_6_Bplus_unconditional source n_inner n_prev p q ε_τ ε_wp h_sign h_fc h_std
  · exact AC.lemma_11_6_Bminus_unconditional source n_inner n_prev p q ε_τ ε_wp h_sign h_fc h_std

  have h_n₀' : n_inner - (ILS.sign E).1 - (ILS.sign E).2 + n_prev ≥ 0 := by
    rw [h_sign]; exact h_n₀
  exact ILS.lemma_11_5_D_unconditional E n_inner n_prev p q p_t q_t h_fc' h_n₀' h_pt h_qt γ₁
